...
public class SecurityConfig {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		http.authorizeHttpRequests(authorize -> authorize
				.requestMatchers("/css/**", "/js/**", "/images/**", "/favicon.ico", "/bookBot/**", "/bot/**",
						"/topic/**", "/message/**")
				.permitAll().requestMatchers("/", "/groupSave", "/login", "/logout", "/signup", "/login?error=true")
				.permitAll() // 로그인 페이지와 오류 페이지에 대한 접근 허용
				.requestMatchers(HttpMethod.DELETE, "/delete/**").hasRole("USER") // DELETE 메서드에 대해 명시적으로 설정
				.requestMatchers("/mypage/", "/reviews", "/upload-temp").hasRole("USER").anyRequest().authenticated())
