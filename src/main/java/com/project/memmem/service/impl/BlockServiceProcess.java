...
public class BlockServiceProcess implements BlockService{
	...
	public void blockUser(Long blockerId, Long blockedId) {
		UserEntity blocker = userRepository.findById(blockerId).orElseThrow(() -> new RuntimeException("Blocker not found"));
        UserEntity blocked = userRepository.findById(blockedId).orElseThrow(() -> new RuntimeException("Blocked user not found"));

        if (!blockRepository.existsByBlockerAndBlocked(blocker, blocked)) {
            BlockListEntity blockListEntity = BlockListEntity.builder()
                    .blocker(blocker)
                    .blocked(blocked)
