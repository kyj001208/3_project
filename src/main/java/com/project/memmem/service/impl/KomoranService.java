...
public class KomoranService {
    ...
    public MessageDTO nlpAnalyze(String message) {
        ...
        Set<String> properNouns = result.getTokenList().stream()
                .filter(token -> "NNP".equals(token.getPos()))
    ...
    private MessageDTO analyzeToken(Set<String> properNouns) {
        ...
        String analyzedContent = "다음과 같은 고유 명사를 분석했습니다: " 
            + String.join(", ", properNouns);
