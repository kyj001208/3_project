...
public class GroupServiceProcess implements GroupService {
	...
	private void saveImages(GroupEntity groups, GroupSaveDTO dto) {
		// 메인 이미지가 있는 경우 저장
		if (dto.getMainImageBucketKey() != null && !dto.getMainImageBucketKey().isEmpty()) {
			ImageEntity mainImage = ImageEntity.builder().groups(groups).imageUrl(dto.getMainImageBucketKey()) // 이미지
																												// URL
																												// 저장
	...
	public List<GroupListDTO> getGroupsByGroupId(Long groupId) {
		return groupRepository.findById(groupId).stream().map(group -> group.toGroupListDTO(baseUrl)) // DTO로 변환
	...
	public boolean isUserCreatorOfGroup(long userId, Long groupId) {
		...
		Optional<GroupMemberShipEntity> membership = groupMemberShipRepository.findByUserUserIdAndGroup_Id(userId,
				groupId);
		...
		if (groupImage != null && !groupImage.isEmpty()) {
			try {
				...
				if (!finalImageUrls.isEmpty()) {
					...
					// 새 이미지 엔티티 생성 및 저장
					ImageEntity newImage = ImageEntity.builder().groups(group).imageUrl(finalImageUrls.get(0))
							.imageType(ImageEntity.ImageType.GROUP).build();
	...
	public Map<String, String> getInitialsForUserAndCreator(Long groupId, long userId) {
		...
		Optional<GroupMemberShipEntity> membershipOpt = groupMemberShipRepository.findByUserUserIdAndGroup_Id(userId,
				groupId);
	...
	public NoticeEntity addNoticeProcess(Long groupId, long userId, NoticeSaveDTO noticeDTO) {
		...
		// 공지사항 엔터티를 생성하고 그룹에 추가
		NoticeEntity notice = noticeDTO.toEntity(group);
		group.addNotice(notice); // 그룹에 공지사항 추가

		return noticeRepository.save(notice);
	}

	public List<NoticeEntity> getNoticesByGroupId(Long groupId) {
		return noticeRepository.findByGroupId(groupId); // 그룹 ID로 공지사항 조회
	}
