...
public class GroupEntity {
	...
	private List<NoticeEntity> notices = new ArrayList<>(); // 여러 공지사항을 담는 리스트
	...
	public GroupListDTO toGroupListDTO(String baseUrl) {
		...
		return GroupListDTO.builder().id(id).groupName(this.groupName).greeting(this.greeting)
				.description(this.description).category(this.category).creatorNickname(creator.getNickName())
	...
	public GroupDTO toGroupDTO(String baseUrl) {
		...
		return GroupDTO.builder().id(this.id).groupName(this.groupName).greeting(this.greeting)
				.categoryKoName(this.category.getKoName()) // Category의 한국어 이름 설정
				.creatorUserId(this.creator.getUserId()).mainImageUrl(getMainImageUrl(baseUrl)) // 메인 이미지 URL 설정
																								// (getMainImageUrl 메서드
																								// 사용)
				.createdAt(formattedDate).build();
	}

	private String getMainImageUrl(String baseUrl) {
		// 이미지 목록 중에서 첫 번째로 찾은 그룹 이미지의 URL을 반환 (없으면 null 반환)
		return images.stream()
				.filter(image -> image.getImageType() == ImageEntity.ImageType.GROUP && image.getImageUrl() != null) // 그룹
																														// 이미지
																														// 타입이면서
																														// URL이
																														// 존재하는
	...
	public void updateImages(List<String> imageUrls, ImageEntityRepository imageRepository) {
		...
		for (String imageUrl : imageUrls) {
			ImageEntity newImage = ImageEntity.builder().groups(this).imageUrl(imageUrl)
